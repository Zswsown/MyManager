// miniprogram/pages/ppt/ppt.js

Page({

  //选择ppt
  choosePPT() {
    if (this.data.pptCount == 0) {
      wx.showToast({
        icon: "none",
        title: '最多只能添加9个ppt！',
      })
      ...
      wx.chooseMessageFile({
        type: 'file',
        count: this.data.pptCount,
        success: (result) => {
          this.setData({
            srcList: this.data.srcList.concat(result.tempFiles),
            pptCount: this.data.pptCount - result.tempFiles.length,
          })
      ...
      for (let i = 0; i < this.data.srcList.length; i++) {
        promiseArr.push(new Promise((reslove, reject) => {
          ...
          if (fileExtension == 'pptx' || fileExtension == 'ppt') {
            ...
            wx.cloud.uploadFile({
              cloudPath: "ppt/" + (item.name), // 上传至云端的路径
              ...
              success: res => {
                ...
                //将添加的ppt的fileID保存为一个集合，放在数据库中
                db.collection('pptFileIDList').add({
                  data: {
                    pptFileName: item.name,
                    pptFileID: res.fileID
                  },
                  success(res) {
                ...
                console.log(res.fileID) //输出上传后ppt的返回地址
                reslove();
                wx.hideLoading();
                wx.showToast({
                  title: "上传成功",
